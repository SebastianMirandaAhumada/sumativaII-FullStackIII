...
public class SecurityConfig {
    ...
    @Autowired
    private CustomAuthentication customAuthentication; // Inyectar el manejador de 401
    @Autowired
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        return http
                ...
                .authorizeHttpRequests(auth -> auth                        
                        ...
                        .requestMatchers(HttpMethod.GET, "/api/libros/**", "/api/users/**").hasAnyRole("USER", "ADMIN") // GET
                                                                                                                     // permitido
                        .requestMatchers(HttpMethod.POST, "/api/libros/**", "/api/users/**").hasRole("ADMIN") // POST
                                                                                                           // permitido
                                                                                                           // solo para
                        // ADMIN
                        .requestMatchers(HttpMethod.PUT, "/api/libros/**", "/api/users/**").hasRole("ADMIN") // PUT
                                                                                                          // permitido
                                                                                                          // solo para
                        // ADMIN
                        .requestMatchers(HttpMethod.DELETE, "/api/libros/**", "/api/users/**").hasRole("ADMIN") // DELETE
                                                                                                             // permitido
                                                                                                             // solo
                        .anyRequest().authenticated() // Cualquier otra ruta requiere autenticación
                )
                .exceptionHandling(ex -> ex
                        .authenticationEntryPoint(customAuthentication) // Usar manejador personalizado para
                                                                        // 401 Unauthorized
...
public CorsConfigurationSource corsConfigurationSource() {
    ...
    configuration.addAllowedOrigin("http://localhost:54820"); // URL del frontend
